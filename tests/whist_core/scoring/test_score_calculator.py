...
class TestScoreCalculator(TeamBaseTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.table_player_a = PlayerAtTable(
            team=0, player=self.player_a, hand=UnorderedCardContainer.empty()
        )
        self.table_player_b = PlayerAtTable(
            team=0, player=self.player_b, hand=UnorderedCardContainer.empty()
        )
        self.table_player_c = PlayerAtTable(
            team=1, player=self.player_c, hand=UnorderedCardContainer.empty()
        )
        self.table_player_d = PlayerAtTable(
            team=1, player=self.player_d, hand=UnorderedCardContainer.empty()
        )
