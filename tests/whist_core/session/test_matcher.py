...
class MatchTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.random_matcher = RandomMatcher()
        self.robin_matcher = RoundRobinMatcher()
    ...
    def test_round_robin_min_player_distribute(self):
        ...
        robin_matcher = RoundRobinMatcher()
        with self.assertRaises(NotEnoughPlayersError):
            _ = robin_matcher.distribute(2, 2, user_list)

    def test_zero_players_per_team(self):
        with self.assertRaises(ValueError):
            _ = self.robin_matcher.distribute(num_teams=2, team_size=0, users=self.user_list)
